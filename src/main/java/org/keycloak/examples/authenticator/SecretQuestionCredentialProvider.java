...
//    public static final String SECRET_QUESTION = "SECRET_QUESTION";
    public static final String SECRET_QUESTION = "SECRET_QUESTION_145";
     public static final String SECRET_QUESTION_2 = "ANTON";
    public static final String CACHE_KEY = SecretQuestionCredentialProvider.class.getName() + "." + SECRET_QUESTION_2;
    ...
    public CredentialModel getSecret(RealmModel realm, UserModel user) {
        ...
        if (user instanceof CachedUserModel) {
            ...
            List<CredentialModel> creds = session.userCredentialManager().getStoredCredentialsByType(realm, user, SECRET_QUESTION_2);
    ...
    public void disableCredentialType(RealmModel realm, UserModel user, String credentialType) {
        if (!SECRET_QUESTION_2.equals(credentialType)) return;

        List<CredentialModel> credentials = session.userCredentialManager().getStoredCredentialsByType(realm, user, SECRET_QUESTION_2);
    ...
    public Set<String> getDisableableCredentialTypes(RealmModel realm, UserModel user) {
        if (!session.userCredentialManager().getStoredCredentialsByType(realm, user, SECRET_QUESTION_2).isEmpty()) {
            Set<String> set = new HashSet<>();
            set.add(SECRET_QUESTION_2);
            return set;
        } else {
    ...
    public boolean supportsCredentialType(String credentialType) {
        return SECRET_QUESTION_2.equals(credentialType);
    }

    @Override
    public boolean isConfiguredFor(RealmModel realm, UserModel user, String credentialType) {
        if (!SECRET_QUESTION_2.equals(credentialType)) return false;
        return getSecret(realm, user) != null;
    }

    @Override
    public boolean isValid(RealmModel realm, UserModel user, CredentialInput input) {
        if (!SECRET_QUESTION_2.equals(input.getType())) return false;
        if (!(input instanceof UserCredentialModel)) return false;
